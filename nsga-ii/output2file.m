...
%*************************************************************************
% 4. Create File with Pareto front info only
%*************************************************************************

    fid2 = fopen([opt.outputfile_dir,'/gen',num2str(state.currentGen),'.txt'], 'w');
    opt.outputfile_paretoFID = fid2;
    
    for p = 1 : opt.popsize
        for i = 1:opt.numObj
            fprintf(fid2, '%.10g\t', pop(p).obj(i) );
        end
        fprintf(fid2, '\r\n');
    end
    fclose(fid2);
